...
export const CalibrationModal: React.FC<CalibrationModalProps> = ({
  ...
  }, [isTriggerPressed, isOpen, step, isCompleted, mode, currentRawX, currentRawY, lastTriggerState, recordCorner]);

  // Auto-Detect Sensitivity: 3-Second Hover Sampling Engine
  useEffect(() => {
    if (!isOpen || isCompleted || mode !== 'auto-detect') return;
  ...
  }, [isOpen, isCompleted, mode, step, isHovering, currentRawX, currentRawY]);

  // When hover time reaches 3 seconds in auto-detect mode, automatically lock corner
  useEffect(() => {
    if (mode === 'auto-detect' && !isCompleted && hoverTimeMs >= HOVER_REQUIRED_MS) {
      recordCorner(step, currentRawX, currentRawY, cornerSamples);
    }
  }, [hoverTimeMs, mode, isCompleted, step, currentRawX, currentRawY, cornerSamples, recordCorner]);

  // Keyboard navigation: Space / Enter to shoot/simulate, Escape to exit
  useEffect(() => {
    if (!isOpen) return;
  ...
  }, [isOpen, isCompleted, step, mode, currentRawX, currentRawY, onClose, recordCorner]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-xs text-neutral-400 mt-0.5">
              Koryguje zniekształcenia perspektywiczne i wyznacza optymalne zakresy wejściowe matrycy
            </p>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <div className="flex items-center bg-neutral-900/90 border border-neutral-800 rounded-lg p-1">
              ...
              >
                ...
                <span>Auto-Wykrywanie Czułości</span>
              </button>
            </div>
      ...
      >
        ...
        {!isCompleted ? (
          <div 
            ...
            className="text-center max-w-lg bg-neutral-900/90 border border-neutral-800 p-5 rounded-xl backdrop-blur relative z-10 shadow-2xl space-y-3"
          >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between text-xs text-neutral-400 font-mono gap-2 pt-1 border-t border-neutral-900">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1 text-cyan-400">
            ...
            Tryb: {mode === 'auto-detect' ? 'Auto-Wykrywanie Czułości (3s Hover)' : 'Manualny Strzał (Trigger/Click)'}
          </span>
          <span className="hidden md:inline text-neutral-600">|</span>
          <span className="hidden md:inline text-neutral-400">
            Bieżący cel: <strong className="text-white">{currentCorner?.name}</strong>
          </span>
        </div>
        <div className="flex items-center gap-3 text-neutral-500">
          <span>SPACJA / ENTER = Strzał / Próbkuj</span>
